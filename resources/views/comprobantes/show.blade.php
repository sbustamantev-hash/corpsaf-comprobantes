        ...
        @if((Auth::user()->isAdmin() || Auth::user()->isAreaAdmin()) && $comprobante->anticipo_id)
            ...
            @if(!in_array($anticipo->estado, ['aprobado', 'rechazado']))
                <div class="flex items-center space-x-3">
                    <button type="button" onclick="openCorrectionModal()"
...
@section('content')
    ...
    <div class="flex flex-col lg:flex-row gap-6">
                    ...
                    <form action="{{ route('comprobantes.observacion', $comprobante->id) }}" method="POST">
                        ...
                        <div class="mb-4">
                            ...
                            <textarea name="mensaje" rows="4"
                                ...
                                required minlength="2" placeholder="Indica qué debe corregir el usuario..."></textarea>
                            @error('mensaje')
                ...
                <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
                    ...
                    <div class="flex flex-wrap gap-4">
                        @if(in_array(strtolower(pathinfo($comprobante->archivo, PATHINFO_EXTENSION)), ['jpg', 'jpeg', 'png', 'gif']))
                            <a href="{{ route('comprobantes.download', $comprobante->id) }}" target="_blank" class="block">
                                <img src="{{ route('comprobantes.download', $comprobante->id) }}" alt="Comprobante"
                                    class="w-24 h-24 object-cover rounded-lg border border-gray-200 hover:border-blue-400 transition cursor-pointer shadow-sm">
                            </a>
                        @else
                            <a href="{{ route('comprobantes.download', $comprobante->id) }}" target="_blank"
        ...
        <div class="lg:w-2/5 lg:min-w-[400px]">
            <div class="bg-white rounded-lg shadow-sm border border-gray-200 flex flex-col sticky top-6"
                        ...
                        @foreach($comprobante->observaciones as $observacion)
                            <div class="flex items-start space-x-3">
                                ...
                                <div class="flex-1 min-w-0">
                                    ...
                                    <p class="text-xs text-gray-500 mb-1">{{ $observacion->created_at->format('M d, Y - h:i A') }}
                                    </p>
                                    ...
                                    @if($observacion->archivo)
                                        <div class="mt-2">
                                            @if(in_array(strtolower(pathinfo($observacion->archivo, PATHINFO_EXTENSION)), ['jpg', 'jpeg', 'png', 'gif']))
                                                <a href="{{ route('observaciones.download', $observacion->id) }}" target="_blank"
                                                    class="block">
                                                    <img src="{{ route('observaciones.download', $observacion->id) }}" alt="Archivo adjunto"
                    ...
                    <form action="{{ route('comprobantes.observacion', $comprobante->id) }}" method="POST"
                        ...
                        <div class="flex items-center space-x-2">
                            ...
                            <label for="archivo-input"
                                class="w-10 h-10 bg-gray-200 text-gray-600 rounded-full hover:bg-gray-300 transition flex items-center justify-center flex-shrink-0 cursor-pointer">
                                <i class="fas fa-paperclip text-sm"></i>
                            </label>
                            <input type="file" name="archivo" id="archivo-input" accept="image/*,application/pdf"
                                class="hidden" onchange="updateFileName(this)">
                            <button type="submit"
                                class="w-10 h-10 bg-blue-600 text-white rounded-full hover:bg-blue-700 transition flex items-center justify-center flex-shrink-0">
